...
def upload_file():
    ...
    try:
        file_path = os.path.join(os.path.dirname(__file__), secure_filename(file_u.filename))
        file_u.save(file_path)
        question = ask_question_to_user_u(read_pdf(file_path))
        return {"answer": question}
       
    except Exception as e:
        return "Erreur lors de l'enregistrement ou du traitement du fichier : {}".format(str(e))
